(function ($) {
    ...
    function reasonMessage(reason) {
        var messages = {
            ...
            ,invalid_os_id: 'Informe um número de OS válido.'
            ,mapos_context_unavailable: 'Não foi possível confirmar a OS no MapOS agora.'
        };
    ...
    var flowDrafts = {}, flowDraftRevisions = {}, flowChats = {}, flowDefinitions = {};
    ...
    function flowSimulation(d) {
        var options=''; $.each(d.simulation_scenarios || [],function(_,scenario){options+='<option value="'+esc(scenario.key)+'">'+esc(scenario.label)+'</option>';});
        return '<hr><div class="wa-flow-simulator" data-key="'+esc(d.key)+'"><h5>Chat de teste seguro</h5><p class="muted">Converse com uma prévia determinística do fluxo. Usa somente dados fictícios e não consulta clientes, não cria OS, não gera link e não envia WhatsApp.</p><label>Cenário fictício</label><select class="wa-flow-scenario">'+options+'</select><div class="wa-flow-chat-log" aria-live="polite" style="height:280px;overflow:auto;background:#f5f7f9;border:1px solid #d8dde3;border-radius:8px;padding:12px;margin:10px 0"></div><div class="input-append" style="width:100%;display:flex"><input class="wa-flow-chat-input" maxlength="500" style="flex:1" placeholder="Digite como se fosse o cliente"><button class="btn btn-primary wa-flow-chat-send" data-key="'+esc(d.key)+'">Enviar no simulador</button></div><p style="margin-top:8px"><button class="btn btn-mini wa-flow-chat-reset" data-key="'+esc(d.key)+'">Reiniciar conversa</button> <button class="btn btn-mini wa-flow-regression" data-key="'+esc(d.key)+'">Executar testes automáticos</button></p><div class="wa-flow-regression-result"></div><details><summary>Ver decisões técnicas</summary><div class="wa-flow-trace" style="margin-top:10px"></div></details></div>';
    }
    function renderFlowChat(d) {
        var h=''; $.each(d.chat || [], function(_,turn) { var client=turn.role==='CLIENT', align=client?'right':'left', color=client?'#d9edf7':'#fff', label=client?'Cliente fictício':'Bot (prévia)'; h+='<div style="text-align:'+align+';margin:7px 0"><div style="display:inline-block;max-width:82%;text-align:left;background:'+color+';border:1px solid #d8dde3;border-radius:10px;padding:9px 11px;white-space:pre-line"><small><strong>'+label+'</strong></small><br>'+esc(turn.text)+'</div></div>'; }); return h || '<p class="muted">Iniciando simulação...</p>';
    }
    ...
    function simulateFlowChat(key, box) {
        var state=flowChats[key] || {scenario:box.find('.wa-flow-scenario').val(),messages:[]}; flowChats[key]=state;
        request('/fluxo/' + encodeURIComponent(key) + '/simular', 'POST', {scenario:state.scenario,messages:state.messages}, function(d) { var log=box.find('.wa-flow-chat-log'); log.html(renderFlowChat(d)); log.scrollTop(log.prop('scrollHeight')); box.find('.wa-flow-trace').html(renderFlowTrace(d)); $('#wa-flow-diagram').html(flowDiagram(flowDrafts[key] || flowDefinitions[key] || {},d.current_node)); });
    }
    function startFlowChat(key) { var box=$('.wa-flow-simulator[data-key="'+key+'"]'); if(!box.length){return;} flowChats[key]={scenario:box.find('.wa-flow-scenario').val(),messages:[]}; simulateFlowChat(key,box); }
    function regressionCases(key) {
        if(key==='customer_service_main'){return [
            {name:'Saudação abre menu',scenario:'NEW_CUSTOMER',messages:['oi'],node:'MAIN_MENU'},
            {name:'Assistência entra no intake',scenario:'NEW_CUSTOMER',messages:['2'],node:'INTAKE'},
            {name:'Hold silencia mensagem comum',scenario:'HUMAN_LOCK',messages:['ainda tenho uma dúvida'],node:'SILENT'},
            {name:'Menu remove hold no teste',scenario:'HUMAN_LOCK',messages:['menu'],node:'MAIN_MENU'}
        ];}
        if(key==='intake'){return [
            {name:'Coleta chega à confirmação',scenario:'NEW_CUSTOMER',messages:['Cliente fictício','notebook','Lenovo','não sei','não liga','coleta','Antonina'],node:'WAITING_CONFIRMATION'},
            {name:'Cliente existente não pede nome',scenario:'EXISTING_CUSTOMER',messages:[],node:'WAITING_DEVICE_TYPE'},
            {name:'Pedido humano interrompe automação',scenario:'NEW_CUSTOMER',messages:['atendente'],node:'HUMAN_REQUESTED'}
        ];}
        if(key==='status_notifications'){return [
            {name:'Regra ativa prepara envio',scenario:'NOTIFICATION_ENABLED',messages:[],node:'SEND'},
            {name:'Regra desativada encerra sem envio',scenario:'NOTIFICATION_DISABLED',messages:[],node:'END'},
            {name:'Atendimento humano mantém transacional na fila',scenario:'TRANSACTIONAL_HELD',messages:[],node:'QUEUE'},
            {name:'MapOS indisponível não presume contexto',scenario:'MAPOS_OFFLINE',messages:[],node:'HANDOFF'}
        ];}
        if(key==='logistics_location'){return [
            {name:'Link aguarda captura consentida',scenario:'LOCATION_PENDING',messages:[],node:'GPS_CAPTURE'},
            {name:'Local preciso permite agendamento',scenario:'LOCATION_ACCURATE',messages:[],node:'SCHEDULE'},
            {name:'Baixa precisão exige revisão',scenario:'LOCATION_LOW_ACCURACY',messages:[],node:'LOCATION_REVIEW'},
            {name:'Sem mapa preserva endereço manual',scenario:'MAPS_UNAVAILABLE',messages:[],node:'GPS_CAPTURE'}
        ];}
        return [{name:'Execução permanece sem efeitos',scenario:'NEW_CUSTOMER',messages:[],node:'START'}];
    }
    function runFlowRegression(key,box) { var cases=regressionCases(key), result=box.find('.wa-flow-regression-result').html('<p>Executando '+cases.length+' testes...</p>'), rows=[],finished=0; $.each(cases,function(_,test){request('/fluxo/'+encodeURIComponent(key)+'/simular','POST',{scenario:test.scenario,messages:test.messages},function(d){var passed=d.current_node===test.node&&d.side_effects===false;rows.push('<tr><td>'+esc(test.name)+'</td><td>'+(passed?'APROVADO':'FALHOU')+'</td><td><code>'+esc(d.current_node)+'</code></td></tr>');finished+=1;if(finished===cases.length){result.html('<table class="table table-bordered table-condensed"><thead><tr><th>Roteiro</th><th>Resultado</th><th>Estado final</th></tr></thead><tbody>'+rows.join('')+'</tbody></table>');}});}); }
    function loadFlow(key, currentNode) { request('/fluxo/' + encodeURIComponent(key), 'GET', null, function(d) {
        flowDefinitions[key]=d;
        ...
        var simulation=d.enabled ? flowSimulation(d) : '<div class="alert alert-info">Este fluxo está planejado; o NLU ainda não participa do atendimento real.</div>';
        ...
        $('#wa-flow-detail').html(h); loadFlowHistory(key, d.revision); if(d.draft_version){loadFlowDraft(key);} startFlowChat(key);
    }); }
    ...
    $(document).on('click', '.wa-flow-chat-send', function () { var button=$(this), box=button.closest('.wa-flow-simulator'), key=button.data('key'), input=box.find('.wa-flow-chat-input'), value=$.trim(input.val()); if(!value){return;} flowChats[key].messages.push(value); if(flowChats[key].messages.length>10){flowChats[key].messages.shift();} input.val(''); simulateFlowChat(key,box); });
    $(document).on('keydown', '.wa-flow-chat-input', function (event) { if(event.keyCode===13&&!event.shiftKey){event.preventDefault();$(this).siblings('.wa-flow-chat-send').trigger('click');} });
    $(document).on('change', '.wa-flow-scenario', function () { var box=$(this).closest('.wa-flow-simulator'), key=box.data('key'); flowChats[key]={scenario:$(this).val(),messages:[]}; simulateFlowChat(key,box); });
    $(document).on('click', '.wa-flow-chat-reset', function () { var button=$(this),box=button.closest('.wa-flow-simulator'),key=button.data('key');flowChats[key]={scenario:box.find('.wa-flow-scenario').val(),messages:[]};simulateFlowChat(key,box); });
    $(document).on('click', '.wa-flow-regression', function () { var button=$(this);runFlowRegression(button.data('key'),button.closest('.wa-flow-simulator')); });
