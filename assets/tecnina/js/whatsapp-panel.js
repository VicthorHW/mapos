(function ($) {
    ...
    function flowSimulation(key) {
        return '<hr><div class="wa-flow-simulator" data-key="'+esc(key)+'"><h5>Chat de teste seguro</h5><p class="muted">Converse com uma prévia determinística do fluxo. Usa somente dados fictícios e não consulta clientes, não cria OS, não gera link e não envia WhatsApp.</p><label>Cenário fictício</label><select class="wa-flow-scenario"><option value="NEW_CUSTOMER">Novo cliente</option><option value="EXISTING_CUSTOMER">Cliente existente</option><option value="AMBIGUOUS_CUSTOMER">Telefone ambíguo</option><option value="HUMAN_LOCK">Atendimento em pausa</option><option value="MAPOS_OFFLINE">MapOS indisponível</option><option value="PICKUP">Pré-atendimento com coleta</option><option value="DROP_OFF">Cliente traz o equipamento</option><option value="LOCATION_PENDING">Localização pendente</option><option value="FALLBACK">Mensagem não compreendida</option></select><div class="wa-flow-chat-log" aria-live="polite" style="height:280px;overflow:auto;background:#f5f7f9;border:1px solid #d8dde3;border-radius:8px;padding:12px;margin:10px 0"></div><div class="input-append" style="width:100%;display:flex"><input class="wa-flow-chat-input" maxlength="500" style="flex:1" placeholder="Digite como se fosse o cliente"><button class="btn btn-primary wa-flow-chat-send" data-key="'+esc(key)+'">Enviar no simulador</button></div><p style="margin-top:8px"><button class="btn btn-mini wa-flow-chat-reset" data-key="'+esc(key)+'">Reiniciar conversa</button> <button class="btn btn-mini wa-flow-regression" data-key="'+esc(key)+'">Executar testes automáticos</button></p><div class="wa-flow-regression-result"></div><details><summary>Ver decisões técnicas</summary><div class="wa-flow-trace" style="margin-top:10px"></div></details></div>';
    }
