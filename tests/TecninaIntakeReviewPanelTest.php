...
expectIntakePanel(strpos($controller, "['save', 'reject', 'approve', 'pickup-fee']") !== false, 'Ações da revisão não estão restritas.');
...
expectIntakePanel(strpos($script, 'www.openstreetmap.org/export/embed.html') !== false, 'Mapa GPS sem API paga não está disponível na revisão.');
expectIntakePanel(strpos($script, 'www.google.com/maps?q=') !== false, 'Link para abrir as coordenadas no Google Maps está ausente.');
expectIntakePanel(strpos($script, 'data.gps_latitude') !== false && strpos($script, 'data.gps_longitude') !== false, 'Coordenadas do detalhe não são renderizadas.');
...
expectIntakePanel(strpos($panel, 'wa-intake-offer-fee') !== false, 'Cotação manual da coleta não pode ser enviada pelo painel.');
expectIntakePanel(strpos($controller, "'/pickup-fee'") !== false, 'Proxy da cotação manual não usa o contrato interno do Gateway.');
expectIntakePanel(strpos($panel, 'credentialSummary') !== false, 'Situação segura da credencial não aparece na revisão.');
