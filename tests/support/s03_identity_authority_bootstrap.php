...
function tecnina_s03_set_reset_attempts($token,$attempts){global $ci;$digest=hash_hmac('sha256',$token,$_ENV['TECNINA_IDENTITY_HMAC_SECRET']);$ci->db->where('token_digest',$digest)->update('tecnina_password_resets',['attempts'=>$attempts]);}
function tecnina_s03_reset_attempts($token){global $ci;$digest=hash_hmac('sha256',$token,$_ENV['TECNINA_IDENTITY_HMAC_SECRET']);$r=$ci->db->select('attempts')->get_where('tecnina_password_resets',['token_digest'=>$digest])->row();return $r?(int)$r->attempts:0;}
