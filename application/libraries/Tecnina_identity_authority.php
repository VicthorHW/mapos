...
class Tecnina_identity_authority
{
    ...
    {
        $lookup=$this->lookupCanonicalPhone($input['canonical_phone']??'');$key=$input['idempotency_key']??null;if(!$lookup['ok']||$lookup['match']!=='UNIQUE'||$lookup['client_id']!==(int)($input['client_id']??0)||!$this->key($key))return ['ok'=>true,'state'=>'REQUEST_ACCEPTED'];
