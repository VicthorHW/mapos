...
| MAP-19 | S03-A identity and credential authority foundation | TL_ACCEPTANCE | additive migration, `Tecnina_identity_authority`, private identity controller, public reset boundary, DB-backed limiter and Nginx safe access-log mapping | production ledger `20260916120000`; TO 56 closures: token lifetime rate limit (10 attempts/token in `attempts`), strict subject validation (422), true multi-connection verify concurrency serialization, upstream Traefik proxy audit, 129/129 target assertions PASS |
